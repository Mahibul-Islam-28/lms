        ...
        <form method="post" class="mx-4 my-2">
            ...
            <div class="form-group my-2">
                ...
                <select name="memberID" id="memberID" class="form-select mt-1" required>
                    ...
                    <option value="{{$member->member_id}}" {{ $borrow->member_id == $member->member_id ? 'selected' : '' }}> {{$member->first_name . ' ' . $member->last_name}} </option>
                    @endforeach
                </select>
            </div>
            <div class="form-group my-2">
                ...
                <select name="bookID" id="bookID" class="form-select mt-1" required>
                    ...
                    <option value="{{$book->book_id}}" {{ $borrow->book_id == $book->book_id ? 'selected' : '' }}> {{$book->title}} </option>
                    @endforeach
                </select>
            </div>
            <div class="form-group my-3">
                <label for="borrowDate">Borrow Date</label>
                <input type="date" name="borrowDate" id="borrowDate" class="form-control mt-2" value="{{$borrow->borrow_date}}">
            </div>
            <div class="form-group my-3">
                <label for="returnDate">Return Date</label>
                <input type="date" name="returnDate" id="returnDate" class="form-control mt-2" value="{{$borrow->return_date}}">
            </div>
            <div class="form-group my-2">
                ...
                <select name="status" id="status" class="form-select mt-1" required>
                    ...
                    <option value="Overdue" {{ $borrow->status == 'Overdue' ? 'selected' : '' }}> Overdue </Overdue>
                </select>
            </div>
